...
const std::type_info& DirectFrame::_type_handle(typeid(DirectFrame));
...
void DirectFrame::set_text(const std::vector<std::string>& text_list)
{
	...
	{
		const std::string& component_name = std::string("text") + std::to_string(i);
...
void DirectFrame::set_image(const std::vector<std::shared_ptr<ImageInput>>& images)
{
	...
	for (int i = 0; i < _options->num_states; ++i)
	{
		const std::string& component_name = std::string("image") + std::to_string(i);
