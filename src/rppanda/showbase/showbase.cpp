...
ShowBase::ShowBase(PandaFramework* framework, WindowFramework* window_framework): panda_framework_(framework), window_framework_(window_framework)
{
	if (global_showbase)
	{
		nout << "ShowBase was already created!" << std::endl;
		return;
	}
...
{
	return window_framework_->get_render();
}

NodePath ShowBase::get_render_2d(void) const
{
	return window_framework_->get_render_2d();
}

NodePath ShowBase::get_aspect_2d(void) const
{
	return window_framework_->get_aspect_2d();
}

NodePath ShowBase::get_pixel_2d(void) const
{
	return window_framework_->get_pixel_2d();
}
...
void ShowBase::setup_mouse(void)
{
	...
	mouse_watcher = window_framework_->get_mouse();
...
void ShowBase::setup_mouse_cb(void)
{
	...
	window_framework_->enable_keyboard();
}
...
{
	return window_framework_->get_camera_group();
}

NodePath ShowBase::get_cam(int index) const
{
	return NodePath(window_framework_->get_camera(index));
}

Camera* ShowBase::get_cam_node(int index) const
{
	return window_framework_->get_camera(index);
}
...
NodePath ShowBase::get_button_thrower(void) const
{
	...
	return window_framework_->get_mouse().find("kb-events");
}

const NodePath& ShowBase::get_data_root(void) const
{
	return panda_framework_->get_data_root();
}
...
void ShowBase::shutdown(void)
{
	...
	if (task = get_task_mgr()->find_task("ival_loop"))
		get_task_mgr()->remove(task);
}

void ShowBase::disable_mouse(void)
{
	// Note that WindowFramework::setup_trackball.
	NodePath tball2cam = window_framework_->get_mouse().find("trackball/tball2cam");
...
{
	window_framework_->setup_trackball();
}
